...
void InitScreen::CreateBlackCloud(int i){
    ...
    BlackCloudPos.push_back(glm::vec2{-BlackCloudSize[i].x-5, rand()%(WINDOW_HEIGHT-180)-50});
...
void InitScreen::CreateWhiteCloud(int i){
    ...
    WhiteCloudPos.push_back(glm::vec2{WINDOW_WIDTH+WhiteCloudSize[i].x+5, rand()%(WINDOW_HEIGHT-180)-50});
...
void InitScreen::draw(std::shared_ptr<Draw::Draw_2D> Draw2D){
    ...
    for(int i=0;i<TextObj.size();i++){
        if(Util::Input::GetCursorPosition().x+WINDOW_WIDTH/2>m_TextPos[i].x && 
            Util::Input::GetCursorPosition().x+WINDOW_WIDTH/2<m_TextPos[i].x+TextScale[i].x && 
            Util::Input::GetCursorPosition().y+WINDOW_HEIGHT/2>m_TextPos[i].y && 
            Util::Input::GetCursorPosition().y+WINDOW_HEIGHT/2<m_TextPos[i].y+TextScale[i].y){
